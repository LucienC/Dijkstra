window.onload=function(){
	 ...
	 arrive = arrive.toUpperCase();
      if (depart!=null && arrive!=null && depart.length == 1 && arrive.length == 1) {
...
console.log(typeof(arrive));
console.log(typeof(ini[1]));
					...
					if(arrets[k].d == tabPoid[ini[0]][0] || arrets[k].a == tabPoid[ini[0]][0]){//Si il a un fils
						for(var j=0; j<tabPoid.length; j++){//On parcours les fils
							if((arrets[k].a == tabPoid[j][0] || arrets[k].d == tabPoid[j][0])  && tabPoid[j][2] == 0 && ((tabPoid[ini[0]][1]+arrets[k].l)<tabPoid[j][1] || tabPoid[j][1]==10000)){//Si le fils n'a pas déja été parcouru
								...
								tabPoid[j][3] = tabPoid[ini[0]][0];
							
								
								console.log("Le fils =>" +tabPoid[j][0]);
								console.log("Son poid =>" +tabPoid[j][1]);
								console.log("Parcouru? =>" +tabPoid[j][2]);
								console.log("antecedent =>" +tabPoid[j][3]);
							}
						}
					}
					k++;
	...
	function afficheTrajet(poids,depart,arrive){
		...
		while(noeud != depart){
			...
			console.log("Noeud dans boucle : " + noeud);
		}
		console.log("Le poid du trajet :"+dist);
